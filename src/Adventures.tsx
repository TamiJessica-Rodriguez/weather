...
export default function Adventures() {
  ...
  const mockedAdventures: Trips[] = [
    ...
    // Lägg till fler äventyr här om du vill...
  ];

  return (
    <div
      className="bg-cover bg-center bg-fixed flex justify-center flex-col"
      style={{ backgroundImage: `url(${AdventureImage})` }}
    >
      <div className="flex items-center justify-center font-extrabold h-20">
        Choose your favourite!
      </div>
      <div className="grid grid-cols-4 gap-4 p-4 m-4 rounded-md border border-gray-800">
        {/* Iterate over mockedAdventures and render each adventure */}
        {mockedAdventures.map((adventure, index) => (
          <div
            className="flex flex-col gap-3 border border-gray-800 rounded-md p-4"
            key={index}
          >
